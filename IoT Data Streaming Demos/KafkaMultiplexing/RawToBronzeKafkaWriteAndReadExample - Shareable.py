...
# MAGIC # Steps
# MAGIC 
# MAGIC ## 1. Write Sample Data to a Kafka Topic
# MAGIC 
# MAGIC ## 2. Read Data from a Kafka topic (or multiple)
# MAGIC 
# MAGIC ## 3. Save kafka data to a Delta Table as a stream

# COMMAND ----------

# DBTITLE 1,Kafka Severs Config
kafka_bootstrap_servers_tls = "<your_kafka_bootstrap_server_tls>"
kafka_bootstrap_servers_plaintext = "<your_kafka_bootstrap_server_plain_text>"
...
# MAGIC ## TIPS: 
# MAGIC 
# MAGIC <li> 1. Use checkpoints in either s3/adls/or dbfs
...
stream1 = df.readStream.format("delta").load("big_bronze_table")
                  .filter(col("SensorMeasurement") == "SchemaA")
  )
  
  #stream1.writeStream.triggerOne
...
(read_stream
 ...
 .mode("append")
 .option("checkpointLocation", checkpoint_location)
 .option("path", <option_output_file_path>)
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECt * FROM PreBronzeAllSensorsFromKafka;
